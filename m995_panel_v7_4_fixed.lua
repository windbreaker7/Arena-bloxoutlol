    ...
    Callback = function()
        if keyEntered == string.lower(KEY_REQUIRED) or keyEntered == "iloveisrael" then
            ...
            if wrongAttempts < 3 then
                Rayfield:Notify({
                    Title = "Error",
                    Content = "[X] Incorrect key (Attempts: " .. wrongAttempts .. "/3)",
                    Duration = 2,
                })
            else
...
keyTab:CreateLabel("[i] Listen to the Key & Peele rap parody...")
keyTab:CreateLabel("[MIC] One of the funniest sketches ever!")
...
task.delay(2, function()
    ...
    if hooked then
        print("[m995 panel] [OK] Successfully hooked into ACS_Framework.Shoot()")
    end
end)
...
local function createCorpseESPTag(corpse)
    ...
    pcall(function()
        ...
        if isPlayerCorpse then
            ...
            labelText  = "[DEAD] " .. displayName .. " (Player)"
            ...
            labelText  = "[DEAD] " .. corpse.Name .. " (Bot)"
...
RunService.Heartbeat:Connect(function()
    ...
    for target, bgui in pairs(injuryLabels) do
        ...
        label.Text = #injured > 0 and ("[WARN] " .. table.concat(injured, " - ")) or ""
    end
end)
...
ToolTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({
            Title = "ESP",
            Content = on and "[OK] Enabled" or "[X] Disabled",
            Duration = 2,
        })
    end,
})
...
ACSTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            ...
            Rayfield:Notify({
                Title = "No Recoil",
                Content = "[WARN] Limited effect - ACS recoil is server-side",
                Duration = 3,
            })
        end
    end,
...
ACSTab:CreateToggle({
    ...
    Callback = function(enabled)
        if enabled then
            ...
            Rayfield:Notify({
                Title = "No Spread",
                Content = "[WARN] Limited effect - ACS spread is server-side",
                Duration = 3,
            })
        end
    end,
...
local function updateNowPlayingLabel()
    ...
    pcall(function()
        nowPlayingLabel:Set(string.format("[NOTE] %s", track.name))
    end)
    pcall(function()
        statusLabel:Set(string.format(
            ...
            isLooping and "[LOOP] " or "",
            isShuffling and "[SHUF]" or ""
        ))
    end)
end
...
MusicTab:CreateButton({
    Name = "[LIST]  Print Playlist to Console",
    Callback = function()
        if #playlist == 0 then
...
ViewTab:CreateButton({
    Name = "[EXIT]  Exit View  (or press E)",
    Callback = function()
...
ViewTab:CreateButton({
    Name = "[RFRSH]  Refresh Player List",
    Callback = function()
        refreshViewPlayers()
    end,
})
...
FlaggedTab:CreateButton({
    Name = "[LIST]  Print Flags to Console",
    Callback = function()
        local count = 0
...
IsraelTab:CreateLabel("[BLU] You  [GRN] Player  [RED] Bot  [YLW] Loot  [TEAL] Extract")
...
DramaTab:CreateSection("[WARN] TROLLING & DRAMA")
...
DramaTab:CreateButton({
    ...
    Callback = function()
        ...
        print("Estimated Chaos Level: [RED][RED][RED][RED][RED]")
...
DramaTab:CreateLabel("[OK] Use at your own risk!")
...
local premiumStatus = getPremiumStatus() and "[OK] Premium" or "[X] Regular"
...
UserInfoTab:CreateButton({
    ...
    Callback = function()
        ...
        print("Premium Status: " .. (premium and "[OK] Premium" or "[X] Regular"))
...
GameTab:CreateLabel("[PIN] Stats update in real-time")
GameTab:CreateLabel("[WARN] Ping is approximated from network lag")
...
PanelChangelogTab:CreateButton({
    Name = "[LIST] Fetch Panel Changelog",
    Callback = function()
        Rayfield:Notify({
        ...
        task.spawn(function()
            pcall(function()
                ...
                if response then
                    ...
                    Rayfield:Notify({
                        Title = "Panel Changelog",
                        Content = "[OK] Changelog loaded! Check console for details.",
                        Duration = 2,
                    })
                    ...
                    Rayfield:Notify({
                        Title = "Panel Changelog",
                        Content = "[X] Failed to fetch changelog",
                        Duration = 2,
                    })
                end
            end)
...
PanelChangelogTab:CreateButton({
    Name = "[DOC] Print Panel Changelog to Console",
    Callback = function()
...
PanelChangelogTab:CreateLabel("[OK] Decompiler Integration")
PanelChangelogTab:CreateLabel("[OK] Tools & Item Database")
PanelChangelogTab:CreateLabel("[OK] User Info Display")
PanelChangelogTab:CreateLabel("[OK] Drama/Trolling Tab")
PanelChangelogTab:CreateLabel("[OK] Key Authentication")
PanelChangelogTab:CreateLabel("[OK] 19 Tabs Total")
PanelChangelogTab:CreateLabel("")
...
DecompilerTab:CreateButton({
    Name = "[SRCH] Start Script Scanner",
    Callback = function()
        Rayfield:Notify({
            Title = "Decompiler",
        ...
        print("[Decompiler] [OK] MEGGD Script Scanner Mobile loaded!")
        
        Rayfield:Notify({
            Title = "Decompiler",
            Content = "[OK] Script Scanner is now active!",
            Duration = 2,
        })
    end,
})
...
DecompilerTab:CreateButton({
    Name = "[DOC] Print Changelog to Console",
    Callback = function()
...
DecompilerTab:CreateLabel("[NOTE] Features:")
...
DecompilerTab:CreateLabel("[PIN] Latest Version: 1.3.1")
DecompilerTab:CreateLabel("[WARN] Note: Opens in separate GUI window")
